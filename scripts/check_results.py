...
def get_data(path):
    ...
    if os.path.isdir(path):
        # Se for diretório, encontra todos os arquivos .pkl
        filepaths = [os.path.join(path, x) for x in os.listdir(path) if x.endswith(".pkl")]
    elif os.path.isfile(path):
        # Se for arquivo único, usa apenas este
        filepaths = [path]
    else:
        raise RuntimeError
    ...
    # Converte rótulos de nomes de domínio para inteiros
...
print(f"Analisando resultados de {opts.data_file}")
...
print(f"Número de traços: {len(X)}")
print()
print("precisão top1: {:.1f}% (+/- {:.1f}%)".format(top1 * 100, top1_std * 100))
print("precisão top5: {:.1f}% (+/- {:.1f}%)".format(top5 * 100, top5_std * 100))
